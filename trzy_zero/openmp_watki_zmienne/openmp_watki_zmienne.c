...
#pragma omp parallel default(none) shared(a_shared, e_atomic) private(b_private) firstprivate(c_firstprivate )
  {
    ...
    #pragma omp barrier //dodanie barirery w celu usunięcia WAR
    d_local_private = a_shared + c_firstprivate;
    
    #pragma omp critical{ //ochrona sekcji krytycznej dla a_shared
      for(i=0;i<10;i++){
        a_shared ++; 
      }
    }
    ...
    #pragma omp atomic //operacja atomowa dla e_atomic
    for(i=0;i<10;i++){
