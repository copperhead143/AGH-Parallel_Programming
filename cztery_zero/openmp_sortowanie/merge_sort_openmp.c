...
void merge_sort_openmp_2(double* A, int p, int r, int poziom){
  ...
  if(p<r){
    poziom++; //zwiekszenie poziomu rekurencji
    ...
    //lewa czesc tablicy
      #pragma omp task final( poziom > max_poziom ) default(none) firstprivate(A,p,r,q1,poziom) 
          {
            // Sprawdzenie warunku final
            if (omp_in_final()) {
              sortowanie_szybkie(A, p, q1); //jezeli osiagnieto maksymalny poziom rekurencji to wywolujemy sortowanie szybkie
            } else {
              merge_sort_openmp_2(A, p, q1, poziom); //jezeli nie ma maksymalnego poziomu rekurancji to wywolujemy merge sort
            }
          }
    //prawa czesc tablicy
      #pragma omp task final( poziom > max_poziom ) default(none) firstprivate(A,p,r,q1,poziom) 
      ...
      #pragma omp taskwait  //oczekiwanie na zakonczenie

          scal(A,p,q1,r); //scalanie tablicy z czesci
        }
